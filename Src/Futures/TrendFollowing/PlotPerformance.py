...
class PlotPerformance(object):
    @classmethod
    def rets_to_equity(cls, returns, compounding=False):
        # Calculate equity curve from returns. Set the starting value of every curve to 1
        # eq = (1 + returns).cumprod(axis=0)
        if compounding:
            eq = (1 + returns).cumprod(axis=0)  # compounding
        else:
            eq = returns.cumsum(axis=0)  # non-compounding
    ...
    def equity_plot(cls, title, label_bench, returns_bench, label_strat, returns_strat,
        ...
        eq_bench = cls.rets_to_equity(returns_bench, compounding)
        eq_strat = cls.rets_to_equity(returns_strat, compounding)
        ...
        cagr_bench = cls.cagr(eq_bench, days=days, compounding=True) * 100
        cagr_strat = cls.cagr(eq_strat, days=days, compounding=True) * 100
        ...
        performance_name = "CAGRs" if compounding else "AGRs"

        ax[subplot_idx].set_title(
            f'{performance_name}: {cagr_bench:.2f} / {cagr_strat:.2f}, '
    ...
    @classmethod
    def yearly_plot(cls, title, label1, returns_series1, label2='', returns_series2=None,
        ...
        if label2:
            ...
            df = pd.DataFrame({label1: returns_series1})
    ...
    def save_equity_as_file(cls, strategy_rets, full_path):
        eq1 = PlotPerformance.rets_to_equity(strategy_rets, compounding=False)
        df = pd.DataFrame()
        df['close'] = eq1
        df['mp'] = 1
        ...
        df.to_csv(str(filename), index=False, header=True, float_format="%g")
