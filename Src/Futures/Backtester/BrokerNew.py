...
# import pandas as pd
#
# from Futures.Backtester.FutureNew import FutureNew
...
#
#         self._trades: Dict[str, List[TradeNew]] = {}
#         self._current_trades: Dict[str, TradeNew] = {}
#
#     def check_state(self) -> bool:
#         return True
#
#     def trades(self, strategy, instrument) -> List[TradeNew]:
#         key = strategy.id * 10000 + instrument.id
#         return self._trades.get(key, None)
#
#     def add_trade(self, strategy, instrument, trade):
#         key = str(strategy.id) + '_' + str(instrument.id)
...
#         key = str(strategy) + str(instrument)
...
#
#     def _check_stop_loss(self, current_trade: TradeNew, strategy: StrategyBase):
...
#
#     def open_position(self, strategy: StrategyBase, instrument: FutureNew,
...
#
#         trade = TradeNew(strategy=strategy, instrument=instrument,
...
#
#     def close_position(self, strategy: StrategyBase, instrument: FutureNew,
#                        price: float = np.nan, is_stop_loss: bool = False):
#         current_trade = self.get_current_trade(strategy, instrument)
#         assert current_trade is not None, \
#             (f"No open position to be closed, "
#              f"strategy: {strategy}, instrument: {instrument}, date: {strategy.timestamp(instrument)}")
#         exit_date = strategy.timestamp()
#         exit_price = price if price is not np.nan else strategy.close()
